...
function getShoppingCart(ids, productsList) {
  ...
  function getProductsFromCart(ids, productsList) {
	const products = ids.map((id) => // percorre a lista de produtos
	  productsList.find((product) => id === product.id) //retorna apenas aquele que tem o id informado
	);
	return products;
  }
  ...
  function getProducts(productsCart) {
	let products = productsCart.map((product) => ({ //para cada produto, cria um objeto com 2 atributos (name e category)
	  name: product.name,
